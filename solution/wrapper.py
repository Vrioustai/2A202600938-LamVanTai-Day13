...
_ORDER_RE = re.compile(r"(?i)\bmua\b|tong\s*cong|tong\s*bao\s*nhieu|het\s*bao\s*nhieu|tinh\s*tong|thanh\s*toan")
...
def _sanitize_question(question: str) -> str:
    text = redact(question or "")[0]
    for pat in _INJECTION_PATTERNS:
        text = pat.sub("[note-data]", text)
    return text
...
def _discount_pct(question: str, discount: dict | None, trace: list) -> int | None:
    ...
    if discount:
        ...
        return 0
    tools = [s.get("tool") for s in (trace or []) if isinstance(s, dict) and s.get("tool")]
...
def _apply_guardrail(question: str, result: dict, config: dict | None = None) -> dict:
    ...
    if _is_price_inquiry(question) and stock:
        answer = _format_price_info(stock)
        answer, _ = redact(answer)
        out = dict(result)
        out["answer"] = answer
        return out

    if not trace or not _is_order_question(question):
        return result

    total, status = _compute_total(question, trace, config)
    if status != "ok":
        answer = _format_refusal(status)
    else:
        answer = _format_total_answer(total)
